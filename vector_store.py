...
def init_vector_store(persist_directory="./db"):
    ...
    return Chroma(collection_name="testoner", embedding_function=embeddings, persist_directory=persist_directory)

# Load PDF and extract text
def process_pdf(pdf_path):
    docs = []
    pdf_document = fitz.open(pdf_path)  # Open PDF
    file_name = os.path.basename(pdf_path)  # Extract file name
...
def process_json(json_path):
    docs = []
    file_name = os.path.basename(json_path)  # Extract file name
...
def split_documents(docs, chunk_size=500, chunk_overlap=50):
    ...
    return text_splitter.create_documents(
        ...
        metadatas=[doc["metadata"] for doc in docs]  # Ensure metadata is passed
    )
